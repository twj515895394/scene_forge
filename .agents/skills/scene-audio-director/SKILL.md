...
description: 当用户要把 SceneForge 的分镜和表演设计转化为动画电影级声音方案，包括配音方向、拟音、环境音、静默点、配乐主题、音频混合计划、卡通动作声、轻伤喜剧声和声音反差时应使用此技能。
---
...
- 分镜阶段包含 `expressive_storyboard_shots`、`stylized_action_shots` 或 `contrast_storyboard`，需要设计对应卡通动作声、轻伤喜剧声和声音反差
...
5. `scene-design-builder` 的场景与角色设定、`expressive_animation_design`
6. `scene-performance-director` 的表演表和停顿设计、`physical_comedy_performance`、`contrast_performance`、`injury_reaction_performance`
7. `scene-storyboard-director` 的完整分镜、`segments`、`shot_highlights`、`expressive_storyboard_shots`、`stylized_action_shots`、`contrast_storyboard`、`hero_moments`、`bridge_shots`、`continuity_rules`、`prompt_hints`、`blocking_map`、`faction_layout` 和 `prop_state_machines`

如果当前声音设计需要查找卡通动作声、轻伤喜剧声或反差声音模板，可按需读取：
...
9. 对轻中度卡通伤害设计喜剧化声音，如 bump_pop、tiny_alarm、stars_twinkle，不使用骨裂、湿黏伤害声或真实惨叫。
10. 对反差喜剧设计声音反差，例如小自行车铃声、儿童道具声、突兀静默或音乐误导释放。
11. 为场景设计环境音床，避免背景音随机或与情绪脱节。
