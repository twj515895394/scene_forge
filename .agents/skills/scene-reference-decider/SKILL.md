...
3. 如需判断是否读取 source intake，先遵循 `.agents/skills/scene-forge/references/optional-source-intake-rule-v6.md`。
4. 如果存在 `source_intake.status: analyzed`，优先读取黑板中的 `source_intake.topic_gate_handoff_summary`，并按需读取：
   - `inputs/source_intake/source_video_priority_map_v1.md`
   - `inputs/source_intake/topic_gate_handoff_v1.md`
5. 不默认读取 `source_video_analysis_v1.md` 或 `source_video_timeline_v1.md` 全文；只有参考边界无法判断时才读取相关章节，并说明原因。
6. 读取 `references/output-contract.md`，确认 `reference_type` 枚举、边界结构和状态推进规则。
7. 裁定本次 `reference_type`：
   - `original_work`
   - `specific_adaptation`
   - `hybrid_reference`
8. 生成结构化 `reference_boundary`，明确主参考、辅助参考、允许继承项和禁止继承项。
9. 将 `source_intake.content_priority_map` 转成参考边界：
   - `core_must_keep` → `must_keep`
   - `highlight_should_keep` → `should_keep`
   - `safe_to_replace` → `allowed_to_rewrite`
   - `avoid_copying` → `must_avoid`
10. 输出单个 YAML 补丁块，并给出 `risk_notes` 和 `next_action`。
11. 将顶层索引更新建议交回总控 Skill：写入 `reference_type`，推进 `project_status: reference_decided`，推进 `next_stage: scene-asset-checker`。
...
- `.agents/skills/scene-forge/references/optional-source-intake-rule-v6.md`：v6 可选 source intake 读取触发条件和状态机
