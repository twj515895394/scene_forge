...
完整剧本正文继续落盘，黑板只记录摘要、档位、演绎风格、Story Beat、v4 表现力机会点和路径引用。
...
3. 如果顶层 `expressive_animation.enabled` 为 `true`，且当前剧本需要识别动画化动作、轻中度卡通伤害或反差喜剧机会点，可按需读取：
   - `assets/animation-stylization/effect-library.md`
   - `assets/animation-stylization/contrast-comedy-library.md`
   只能用于当前 Story Beat 机会点判断，不得全仓库扫描。
4. 先检查 `user_confirmations.duration_strategy_confirmed` 和 `segment_strategy`。如果整片目标时长、单段视频生成时长或分段策略未确认，先输出时长/分段候选方案，不生成正式剧本。
5. 在剧本正式落盘前，先输出“剧本方案预览”，至少包含改编档位候选、演绎风格候选、目标总时长、Story Beat 草案、保留点、重写点、v4 表现力机会点和风险边界。
6. 等待用户明确确认剧本方案；用户纠错或补充偏好不等于授权落盘。
7. 用户确认后，选择本次改编档位，并结合选题阶段建议最终确认 `performance_style`。
8. 输出适合后续表演导演和分镜使用的剧本摘要、Story Beat、场次拆分建议、v4 表现力机会点和改编说明。
9. 将完整剧本正文写入 `details/script_v*.md`；黑板只记录摘要、档位、演绎风格、Story Beat、关键镜头提示、v4 表现力机会点和路径引用。
10. 输出单个 YAML 补丁块；`summary` 使用中文，必要时附英文参数值。
11. 将状态推进建议交回总控 Skill，进入 `scene-performance-director`。
