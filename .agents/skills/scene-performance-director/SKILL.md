...
5. `scene-design-builder` 的角色设定、视觉语言、角色魅力设计、`expressive_animation_design`、`blocking_map`、`faction_layout` 和 `prop_state_machines`（如有）
6. `scene-script-adapter` 的剧本摘要、完整剧本路径、`story_beats`、`expressive_beat_opportunities`、`stylized_action_opportunities`、`contrast_opportunities`、改编档位、演绎风格、`segment_strategy`、Hero 候选和 Bridge 需求
...
2. 读取剧本摘要、完整剧本路径、`story_beats` 和 v4 表现力机会点。
3. 读取角色设定，提取每个主要角色的性格、轮廓、形状语言、表情梯度和反差边界。
4. 继承 `expressive_animation`、`segment_strategy`、`hero_moment_candidates`、`storyboard_hints.bridge_shot_need`、`blocking_map`、`faction_layout` 和 `prop_state_machines`，作为后续分镜表演连续性的输入。
...
12. 将完整表演表写入 `details/performance_sheet_v*.md`。
13. 黑板只记录摘要、路径、角色表演概要、Beat 表演重点、v4 表现力表演设计和连续性规则。
14. 输出单个 YAML 补丁块，并将状态推进到 `performance_ready`，下一阶段为 `scene-storyboard-director`。
...
- `references/output-contract.md`：表演导演输出协议、v4 表现力表演字段、黑板摘要字段和长正文落盘方式
