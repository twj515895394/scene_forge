...
它重点补齐 Pixar-like 动画电影路线中最关键的表演层：
...
2. `scene-reference-decider` 的参考边界、保留项和规避项
3. `scene-design-builder` 的角色设定、视觉语言和角色魅力设计
4. `scene-script-adapter` 的剧本摘要、完整剧本路径、`story_beats`、改编档位和演绎风格
5. `docs/皮克斯电影风格路线实施计划.md` 中关于表演导演的要求
6. `references/output-contract.md` 中定义的输出协议

## 执行步骤

1. 读取项目 `PROJECT_BOARD.md`，确认 `project_status` 为 `script_ready`。
2. 读取剧本摘要、完整剧本路径和 `story_beats`。
3. 读取角色设定，提取每个主要角色的性格、轮廓、形状语言和表情梯度。
4. 按每个角色建立 `character_performance_profile`。
5. 按每个 Story Beat 设计表演目标、眼神、微表情、身体动作、停顿和次级动作。
6. 明确每个角色至少一个 `signature_gesture`。
7. 明确关键 Beat 的 `pause_or_hold`，尤其是喜剧反应、情绪犹豫和情绪释放点。
8. 将完整表演表写入 `details/performance_sheet_v*.md`。
9. 黑板只记录摘要、路径、角色表演概要、Beat 表演重点和连续性规则。
10. 输出单个 YAML 补丁块，并将状态推进到 `performance_ready`，下一阶段为 `scene-storyboard-director`。
