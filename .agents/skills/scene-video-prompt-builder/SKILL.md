...
2. 采用 `compact` 上下文预算：只读取 `PROJECT_BOARD.md`、本 Skill、`references/output-contract.md`、完整分镜、表演表、声音方案、设计摘要、参考边界、`creative_direction_context` 和必要的 Blocking/道具状态输入；执行期读取边界同时遵循仓库根 `AGENTS.md`。
...
14. 用户确认后，先生成视频提示词草稿：为每个视频分段整合 Beat Skeleton 意图、`primary_vgu_ids`、镜头内容、动作、运镜、镜头语言、视觉动机、表演、按上游对白设计继承的具体台词、拟音、音乐、环境音、静默点、节奏、参考图使用方式、source intake 规避约束、表现力扩展要求、双版故事板转译规则和连续性钩子；若上游提供故事板总板或故事板包，必须把它们视为顺序视觉关键帧参考，而不是松散灵感图。
