...
- 模型投喂版：中文 / 英文各一份，采用 `Segment + Shot + Timecode` 双层结构，直接服务外部视频模型
...
本阶段负责把 `scene-storyboard-director` 的 `storyboard_content_breakdown`、`cinematic_language_plan`、`video_generation_units`、`camera_language`、`visual_motivation`、`selected_shot_pattern`、锚帧和连续性控制结构写入最终 Segment Prompt。
...
- 控制版 / 风格版故事板使用声明
- 模型适配计划
- 双语交付计划：中文长版 / 英文长版 / 中文运行时版 / 英文运行时版
- 全局负向约束
- 需要用户确认的问题
...
- `scene-storyboard-director`：`storyboard_version`、`storyboard_summary`、`storyboard_content_breakdown`、`cinematic_language_plan`、`video_generation_units`、`opening_anchor_frame`、`closing_anchor_frame`、`space_continuity_map`、`action_continuity_chains`、`emotion_continuity_chains`、`segments`、`shot_highlights`、`expressive_storyboard_shots`、`stylized_action_shots`、`contrast_storyboard`、`hero_moments`、`bridge_shots`、`continuity_rules`、`storyboard_prompt_pack_mode`、`storyboard_prompt_files`、`control_storyboard_prompt_file`、`styled_storyboard_prompt_file`、`storyboard_quality_check`、`prompt_hints`、`blocking_map`、`faction_layout`、`prop_state_machines`、`creative_direction_context`
...
- `storyboard_methodology_inheritance`：继承视频生成单元、锚帧、连续性控制、双版故事板和模型适配摘要。
...
- `video_prompt_files`：最终交付文件路径。默认必须支持中文 / 英文双份，以及导演长版 / 模型投喂版双层。
...
- `methodology_refs`：当前提示词真正使用到的视频生成单元、连续性控制和模型适配文件。
...
- `source_video_generation_units` / `source_anchor_frames`：把最终每段 prompt 对应的分镜控制单元和锚帧来源记清，防止末端断链。
...
- `dialogue_plan` 必须显式给出本段的对白存在性判断，而不是默认存在对白；有对白时写说话角色、对白意图、语气和停顿，无对白时写明由动作、表情、环境音或静默承载。
...
- `blocking_execution` 必须把站位说明升级成运行时可执行结构，不能只写“角色在左边、右边”。
...
- `shot_plan` 是当前模型投喂版的强结构要求。模型投喂版不得只保留“10 秒一段的大 Prompt”，而必须显式写出每个 Shot 的时间码和执行重点。
...
outputs/video_prompts/视频提示词_模型投喂版_中文_v*.md
outputs/video_prompts/视频提示词_模型投喂版_英文_v*.md
```

如需要按段拆文件，可使用：

```text
outputs/video_prompts/视频提示词_模型投喂版_中文_segment_01_v*.md
outputs/video_prompts/视频提示词_模型投喂版_英文_segment_01_v*.md
```
