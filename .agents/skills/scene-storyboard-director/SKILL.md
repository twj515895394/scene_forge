...
负责把 Story Beat、source intake 继承约束和表演导演结果转成可执行分镜，强调动画电影化镜头语言，而不是复刻原片或源视频拍法。
...
黑板记录分镜摘要、分段信息、故事板 prompt 路径、Hero Shot、Bridge Shot、v4 表现力镜头、v5 镜头语言、source intake 镜头继承说明和连续性规则，完整分镜另行落盘。
...
2. 采用 `compact` 上下文预算：只读取 `PROJECT_BOARD.md`、本 Skill、`references/output-contract.md`、剧本摘要、`story_beats`、表演表、设计摘要、参考边界和必要的 Blocking/道具状态输入。
...
6. 如果当前分镜需要 v5 专业镜头语言、动画电影镜头模式或动作喜剧分镜 pattern，可按需读取：
...
7. 读取 `references/output-contract.md`，确认分镜字段、三层时间模型、v4 表现力镜头字段、v5 内容拆分与镜头语言字段、黑板摘要和长分镜落盘路径。
8. 先读取顶层 `segment_strategy`、`segment_duration_seconds`、`target_total_duration_seconds`；若为空，必须先向用户确认总时长、10 秒 / 15 秒 / 混合分段策略，不直接生成正式分镜。
9. 在正式落盘前，先输出“分镜方案预览”，至少包含镜头数量建议、Segment Plan、Hero Shot、Bridge Shot、source intake 镜头继承计划、Blocking Map、关键道具状态变化、v4 表现力镜头候选、v5 `storyboard_content_breakdown` 样例、v5 `cinematic_language_plan` 样例和需要用户确认的问题。
10. 等待用户明确确认分镜方案；用户纠错或补充偏好不等于授权落盘。
11. 用户确认后，先生成 `storyboard_content_breakdown`，把每个关键 Story Beat 拆成 action / reaction / emotion_shift / reveal / transition / insert / environment_response / contrast_payoff / stylized_impact / injury_result 等可拍内容单元。
12. 再生成 `cinematic_language_plan`，为每个关键内容单元选择景别、机位、构图、镜头运动、剪辑节奏和 `visual_motivation`。
13. 若使用资产库 pattern，必须写明 `selected_shot_pattern.pattern_id`、`source_asset`、`reason` 和“只使用抽象结构，不复制具体电影镜头”的说明。
14. 生成动画电影化分镜，不直接照搬原版或源视频拍法。
15. 对每个镜头至少明确动作、表情、调度、情绪功能、导演意图、声音意图、镜头语言和时间轴位置。
16. 对动画物理镜头，必须镜头化 setup / impact / deformation / hold / recovery。
17. 对反差喜剧镜头，必须镜头化 setup / conceal / reveal / hold / continue。
18. 对轻中度卡通伤害镜头，必须明确 injury_visibility、喜剧化呈现方式和禁止的写实伤害边界。
19. 产出“每段覆盖哪些镜头和 Story Beat”的 Segment Plan，并写出可直接生成故事板图的 prompt 文件。
20. 将完整分镜写入 `details/shotlist_v*.md`；黑板只记录版本、镜头总览、分段信息、故事板 prompt 路径、Hero Shot、Bridge Shot、Blocking Map、source intake 镜头继承说明、v4 表现力镜头、v5 镜头语言和关键连续性规则。
21. 输出单个 YAML 补丁块；`summary` 使用中文，必要时附英文参数值。
22. 将状态推进建议交回总控 Skill，进入 `scene-audio-director`。
...
- 不照搬源视频原始镜头顺序、机位组合、构图、剪辑节奏、台词字幕画面或强识别视觉表达。
...
- 必须继承 `source_intake` 的核心动作链和规避边界，但不得复刻源视频镜头表达。
- 使用 `assets/cinematic-language/*` 时，只能使用抽象模式，不得在运行时输出“模仿某部电影镜头”。
- 不照搬原版或源视频拍法，要用动画电影化的表演、节奏、视角和情绪调度重构名场面。
