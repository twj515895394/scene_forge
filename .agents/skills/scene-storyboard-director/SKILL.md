...
description: 当用户要把 SceneForge 剧本节拍、source_intake 视频源优先级分层和表演导演结果转成具有动画电影感的专业分镜方案，并沉淀剧本内容拆分、影视镜头语言、镜头级表演、调度、声音意图、动画物理镜头、轻伤可见度和反差喜剧 reveal 时应使用此技能。
---

# scene-storyboard-director

负责把已确认 Story Beat、source intake 继承约束和表演导演结果转成可执行分镜，强调动画电影化镜头语言，而不是复刻原片或源视频拍法。
...
-> video_generation_units / continuity_control
-> shot_highlights / shotlist / storyboard_prompts
```

执行顺序：

1. 先把剧本、source intake 约束和表演拆成可拍的分镜内容单元。
2. 再把内容单元转换成专业影视镜头语言。
3. 再补齐视频生成单元、开头/结尾锚帧、空间连续性和动作/情绪连续性控制。
...
7. 如果项目已把 `assets/storyboard-methodology/*` 加入 `runtime_policy.context_policy.allowed_runtime_asset_paths`，按需读取：
...
8. 读取 `references/output-contract.md`，确认分镜字段、三层时间模型、视频生成单元字段、连续性控制字段、双版故事板字段、黑板摘要和长分镜落盘路径。
9. 先读取当前可用的 `segment_strategy`、`project_config.segment_duration_seconds`、`project_config.target_total_duration_seconds`；若为空，必须先向用户确认总时长、10 秒 / 15 秒 / 混合分段策略，不直接生成正式分镜。
10. 在正式落盘前，先输出“分镜方案预览”，至少包含镜头数量建议、Segment Plan、Hero Shot、Bridge Shot、source intake 镜头继承计划、Blocking Map、关键道具状态变化、`storyboard_content_breakdown` 样例、`cinematic_language_plan` 样例、`video_generation_units` 样例、开头/结尾锚帧策略、空间连续性和动作/情绪连续性策略、故事板是否需要拆成多个 prompt 包，以及需要用户确认的问题。
11. 等待用户明确确认分镜方案；用户纠错或补充偏好不等于授权落盘。
12. 用户确认后，先生成 `storyboard_content_breakdown`，把每个关键 Story Beat 拆成 action / reaction / emotion_shift / reveal / transition / insert / environment_response / contrast_payoff / stylized_impact / injury_result 等可拍内容单元。
13. 再生成 `cinematic_language_plan`，为每个关键内容单元选择景别、机位、构图、镜头运动、剪辑节奏和 `visual_motivation`。
14. 基于前两层结果生成 `video_generation_units`，明确每个视频生成单元的目标时长、入镜 / 出镜、镜头密度、叙事职责和转场钩子。
15. 为关键单元补齐 `opening_anchor_frame`、`closing_anchor_frame`、`space_continuity_map`、`action_continuity_chains` 和 `emotion_continuity_chains`。
16. 若使用资产库 pattern，必须写明 `selected_shot_pattern.pattern_id`、`source_asset`、`reason` 和“只使用抽象结构，不复制具体电影镜头”的说明。
17. 生成动画电影化分镜，不直接照搬原版或源视频拍法。
18. 对每个镜头至少明确动作、表情、调度、情绪功能、导演意图、声音意图、镜头语言和时间轴位置。
19. 对动画物理镜头，必须镜头化 setup / impact / deformation / hold / recovery。
20. 对反差喜剧镜头，必须镜头化 setup / conceal / reveal / hold / continue。
21. 对轻中度卡通伤害镜头，必须明确 injury_visibility、喜剧化呈现方式和禁止的写实伤害边界。
22. 产出“每段覆盖哪些镜头和 Story Beat”的 Segment Plan，并分别写出控制版 / 风格版故事板 prompt 文件。
23. 如果 `total_shots > 20`，或 `target_total_duration_seconds > 90`，默认推荐拆成多个故事板 prompt 包，不把全部镜头硬塞进一个故事板 prompt。
24. 拆分多个故事板 prompt 包时，优先按连续戏剧单元、场景块或段落块拆，而不是机械平均分镜头数。
25. 将完整分镜写入 `details/分镜清单_v*.md`，并把 `video_generation_units`、空间连续性图、动作/情绪连续性链、故事板质量检查分别落到 `details/storyboard/` 或 `outputs/storyboard_prompts/`；黑板只记录版本、索引、摘要和关键连续性规则。
26. 输出单个 YAML 补丁块；`summary` 使用中文，必要时附英文参数值。
27. 将状态推进建议交回总控 Skill，进入 `scene-audio-director`。
...
- 若必须保留动作链，应重构为动画电影化镜头语言，而不是复刻源视频拍法。
...
- 当项目目标是改写时，用动画电影化的表演、节奏、视角和情绪调度重构名场面；当项目目标是保留优先时，允许在此基础上尽量贴近原版镜头表达。
...
- `video_generation_units` 必须和 `segments` 对齐，但不能退化成“每段固定一个镜头”的机械结构。
