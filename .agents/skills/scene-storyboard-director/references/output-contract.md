...
本阶段必须先把剧本与表演拆成可拍的 `storyboard_content_breakdown`，再将内容单元转换成 `cinematic_language_plan`，随后生成 `video_generation_units` 与连续性控制结构，最后从镜头语言计划派生 `shot_highlights`、完整 shotlist 和双版故事板 prompt。
...
data:
  ...
  video_generation_units:
    - vgu_id:
      ...
      opening_anchor_needed: false
      closing_anchor_needed: false
  ...
  downstream_video_prompt_handoff:
    required_output_mode: dual_pack
...
- `storyboard_methodology_inheritance`：继承视频生成单元、连续性控制、双版故事板和质量检查方法论摘要。
...
- `video_generation_units`：把内容单元组织成视频生成友好的镜头控制单元，服务后续视频提示词和参考图喂入。
...
- `continuity_rules`：角色、表演、场景、运动、声音、站位、道具状态、表现力扩展和镜头语言连续性约束。
- `storyboard_quality_check`：故事板阶段的结构化质量检查结果。必须检查镜头职责、动作交接、空间交接、连续性四线和双版故事板一致性。
- `control_storyboard_prompt_file` / `styled_storyboard_prompt_file`：同源双版故事板输出。控制版保结构，风格版保氛围，但两者叙事和镜头职责必须一致。
- `audio_handoff`：交给 `scene-audio-director` 的声音设计提示。
- `prompt_hints`：交给视频提示词阶段的重点。
- `downstream_video_prompt_handoff`：显式告诉视频提示词阶段必须交付什么结构，避免末端自行压缩成说明型大 Prompt。
