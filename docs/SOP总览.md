...
4. 角色与场景设定：根据制作分层决定轻量锁定卡或完整设定，并同时产出可直接生成设定图的 prompt。
5. 剧本改编：支持原样保留、大部分保留、部分改写、重构扩写四档，并由该阶段最终确认 `performance_style`。
6. 分镜设计：强调 3D 动画电影感镜头语言，不死套原版拍法；默认按 `10s` 分段，必要时在该阶段确认改为 `15s`。
7. 故事板与视频提示词：分镜阶段先产出镜头时间轴和故事板提示词；提示词阶段再产出按段整合动作、运镜、台词和音效的视频分段提示词。
...
为了直接服务后续视频生成，后半段三个 Skill 的核心交付物统一为：

1. `scene-design-builder`
   - 角色设定锁定卡
   - 场景设定锁定卡
   - 可直接生成设定图的 prompt 文件
2. `scene-storyboard-director`
   - 带时间轴的镜头表
   - 每段覆盖哪些镜头的分段方案
   - 可直接生成故事板图的 prompt 文件
3. `scene-video-prompt-builder`
   - 按 `segment_duration_seconds` 拆分的视频 prompt
   - 每段整合镜头内容、动作、运镜、台词、音效和节奏

默认 `segment_duration_seconds = 10`；若分镜阶段判断 10 秒承载不下关键表演，可在用户确认后改为 `15`。
